import { loadTheme as p } from "./theme-C1Hn9FBu.js";
import { generateCSS as u } from "./generateCSS-DD50tOKZ.js";
class _ extends HTMLElement {
  ...
  async connectedCallback() {
    const t = await p(), i = u(t), e = document.createElement("template");
    e.innerHTML = `<style>${i}</style>
    ...
    this._onSlotChange(), this._selectedEl.addEventListener("click", (s) => {
      s.stopPropagation(), !(this.hasAttribute("disabled") || this.getAttribute("mode") === "view") && this._togglePopup();
    }), this._selectedEl.addEventListener("keydown", (s) => {
      s.key === "Enter" || s.key === " " ? (s.preventDefault(), this._togglePopup()) : s.key === "ArrowDown" && (s.preventDefault(), this._focusFirstItem());
    }), this._root && this._root.addEventListener("keydown", this._onKeyDown), this._applyDisabledState(), this._applyModeState();
  }
  ...
  _onSlotChange() {
    ...
    i.forEach((s) => {
      try {
        s.tagName && s.tagName.toLowerCase() === "aws-selector" && (s.style.display = "none", s.hidden = !0, s.setAttribute("aria-hidden", "true"));
      } catch {
      }
    });
    const e = i.filter((s) => s.tagName && s.tagName.toLowerCase() === "aws-option");
    this._options = e, e.forEach((s, r) => {
      try {
        s.style.display = "none", s.hidden = !0, s.setAttribute("aria-hidden", "true");
      } catch {
      }
      s.setAttribute("role", "option"), s.hasAttribute("tabindex") || s.setAttribute("tabindex", "-1");
    });
    const o = e.find((s) => s.hasAttribute("selected")) || e[0];
    this._focusedIndex = e.indexOf(o) >= 0 ? e.indexOf(o) : 0, this._updateSelectedLabel(o), this._onDocumentClick || (this._onDocumentClick = (s) => {
      const r = this.contains(s.target) || this.shadowRoot && this.shadowRoot.contains(s.target), l = this._portalEl && this._portalEl.contains(s.target);
      !r && !l && this._closePopup();
    });
  }
  _selectByElement(t) {
    ...
    const i = t.getAttribute("data-id") || t.id || null;
    this.dispatchEvent(new CustomEvent("change", { detail: { id: i, element: t }, bubbles: !0, composed: !0 }));
  }
  _applyDisabledState() {
  ...
  _openPopup() {
    ...
    i.className = "popup open", i.setAttribute("role", "listbox"), i.setAttribute("aria-hidden", "false"), this._options.forEach((l, c) => {
      ...
      d && (n.dataset.id = d), n.addEventListener("click", (a) => {
        a.stopPropagation(), this._selectByElement(l), this._closePopup();
      }), n.addEventListener("keydown", (a) => {
        (a.key === "Enter" || a.key === " ") && (a.preventDefault(), this._selectByElement(l), this._closePopup());
      }), i.appendChild(n);
    });
    try {
    ...
    t.appendChild(i), document.body.appendChild(t), this._portalEl = t;
    try {
    ...
    let o = e.bottom, s = e.left;
    i.style.visibility = "hidden", requestAnimationFrame(() => {
      const l = i.offsetHeight;
      o + l > window.innerHeight && (o = e.top - l), t.style.left = Math.max(4, s) + "px", t.style.top = Math.max(4, o) + "px", i.style.visibility = "visible";
  ...
  _focusFirstItem() {
    if (this._portalEl) {
      const i = this._portalEl.querySelector(".item");
      i && i.focus();
      return;
    }
