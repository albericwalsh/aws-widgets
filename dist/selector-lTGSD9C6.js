import { loadTheme as u, getThemeObject as p } from "./theme-D01i-Ra9.js";
import { generateCSS as _ } from "./generateCSS-Bb8EuG6l.js";
class y extends HTMLElement {
  ...
  async connectedCallback() {
    ...
    const t = await u(), s = _(t), e = document.createElement("template");
  ...
  _onSlotChange() {
    ...
    const l = e.find((i) => i.hasAttribute("selected")) || e[0];
    this._focusedIndex = e.indexOf(l) >= 0 ? e.indexOf(l) : 0, this._updateSelectedLabel(l), this._onDocumentClick || (this._onDocumentClick = (i) => {
      const a = this.contains(i.target) || this.shadowRoot && this.shadowRoot.contains(i.target), o = this._portalEl && this._portalEl.contains(i.target);
      !a && !o && this._closePopup();
    });
  }
  _selectByElement(t) {
  ...
  set value(t) {
    try {
      ...
      const s = this._options.find((e) => {
        const l = e.getAttribute("data-id"), i = e.id || null, a = e.getAttribute("value"), o = (e.textContent || "").trim();
        return l !== null && l == t || i && i == t || a !== null && a == t || o && o == t;
      });
  ...
  _openPopup() {
    ...
    s.className = "popup open", s.setAttribute("role", "listbox"), s.setAttribute("aria-hidden", "false"), this._options.forEach((o, n) => {
      const r = document.createElement("div");
      r.className = "item", r.tabIndex = 0;
      try {
        r.innerHTML = o.innerHTML;
      } catch {
        r.textContent = o.textContent || "";
      }
      const d = o.getAttribute("data-id") || o.id || "";
      d && (r.dataset.id = d), r.addEventListener("click", (c) => {
        c.stopPropagation(), this._selectByElement(o), this._closePopup();
      }), r.addEventListener("keydown", (c) => {
        (c.key === "Enter" || c.key === " ") && (c.preventDefault(), this._selectByElement(o), this._closePopup());
      }), s.appendChild(r);
    });
    try {
      ...
      if (o && o.textContent) {
        ...
        n.textContent = o.textContent, t.appendChild(n);
      }
    } catch {
    }
    ...
    try {
      ...
      Object.keys(o).forEach((n) => {
        typeof o[n] == "string" && n.startsWith("aws-") && t.style.setProperty("--" + n, o[n]);
      });
      try {
        const n = window.getComputedStyle(this), r = n.getPropertyValue("background-color") || n.getPropertyValue("background"), d = n.getPropertyValue("border-color");
        r && r.trim() && t.style.setProperty("--aws-bg", r.trim()), d && d.trim() && t.style.setProperty("--aws-border", d.trim());
        try {
          s.style.background = r && r.trim() ? r.trim() : s.style.background;
        } catch {
        }
      } catch {
      }
    ...
    let l = e.bottom, i = e.left;
    s.style.visibility = "hidden";
    const a = () => {
      const o = this._selectedEl.getBoundingClientRect();
      l = o.bottom, i = o.left;
      const n = s.offsetHeight;
      l + n > window.innerHeight && (l = o.top - n), t.style.left = Math.max(4, i) + "px", t.style.top = Math.max(4, l) + "px", s.style.visibility = "visible";
    };
    requestAnimationFrame(a), this._repositionHandler = () => {
      try {
        a();
      } catch {
      }
    }, window.addEventListener("scroll", this._repositionHandler, !0), window.addEventListener("resize", this._repositionHandler), this._selectedEl.setAttribute("aria-expanded", "true"), this._focusFirstItem();
  }
  _closePopup() {
    try {
      Array.from(document.querySelectorAll(".aws-selector-portal")).forEach((e) => {
        try {
          const l = e.querySelector(".popup");
          l && (l.classList.remove("open"), l.style.display = "none", l.setAttribute("aria-hidden", "true")), e.style.display = "none", e.remove();
        } catch {
        }
    ...
    this._portalEl = null;
    try {
      this._repositionHandler && (window.removeEventListener("scroll", this._repositionHandler, !0), window.removeEventListener("resize", this._repositionHandler), this._repositionHandler = null);
    } catch {
    }
    try {
      document.removeEventListener("click", this._onDocumentClick);
  ...
  _onKeyDown(t) {
    ...
    if (s === "ArrowRight" || s === "ArrowDown")
      if (t.preventDefault(), this._focusedIndex = Math.min(this._options.length - 1, this._focusedIndex + 1), this._portalEl) {
        const l = Array.from(this._portalEl.querySelectorAll(".item"))[this._focusedIndex];
        l && l.focus();
      } else {
...
customElements.get("aws-selector") || customElements.define("aws-selector", y);
